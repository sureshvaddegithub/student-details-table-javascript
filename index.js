...
console.log(tbody[1].innerHTML);

const sorting_buttons = document.querySelectorAll(".sorting-buttons button");

const table =document.querySelectorAll("table");
table[1].style.visibility="hidden";
 
const searchBar = document.getElementById("handlechanger");
...
{"id":100,"first_name":"Zoe","last_name":"Moorhead","email":"zmoorhead2r@sfgate.com","gender":"Female","img_src":"https://robohash.org/fugiatcorporisdeleniti.png?size=50x50&set=set1","class":8,"marks":43,"passing":true,"city":"Makin Village"}]
 ...
 function renderStudents(list,ref){
    if(ref == tbody[0]){
    tbody[0].innerHTML  = "";
    table[1].style.visibility="hidden";
    }
    ...
    list.forEach((Element,index)=>{
        ...
        const Name = document.createElement("td");
         ...
         fullName.innerText = Element.first_name +" "+Element.last_name;
         div.appendChild(img);
        ...
        studentinfo.appendChild(gender);
        studentinfo.appendChild(Class);
        studentinfo.appendChild(marks);
        studentinfo.appendChild(passing);
        studentinfo.appendChild(email);
        
        if(ref === tbody[0])
        tbody[0].appendChild(studentinfo);
         else{
             tbody[1] .appendChild(studentinfo);
         }
...
renderStudents(studentDB,tbody[0]);


function searchString(input){
    let results = studentDB.filter((ele)=>{
        if(ele.first_name.toLowerCase().includes(input))
        return ele;
        else if(ele.last_name.toLowerCase().includes(input))
         return ele;
         else if(ele.email.toLowerCase().includes(input))
         return ele;
    })
    renderStudents(results,tbody[0]);
}
...
sorting_buttons[0].addEventListener("click",()=>{
    let sortByAToZ = studentDB.sort(function(a,b){
    ...
    renderStudents(sortByAToZ,tbody[0]);
})
sorting_buttons[1].addEventListener("click",()=>{
    let sortByZToA = studentDB.sort(function(a,b){
        var nameA = a.first_name+" "+a.last_name;
        var nameB = b.first_name+" "+b.last_name;
        if(nameA > nameB)
        return-1;
    ...
    renderStudents(sortByZToA,tbody[0]);
})
sorting_buttons[2].addEventListener("click",()=>{
    let sortBymarks = studentDB.sort(function(a,b){
        return a.marks-b.marks;
    })
    renderStudents(sortBymarks,tbody[0]);
})

sorting_buttons[3].addEventListener("click",()=>{
    let sortByPassing = studentDB.filter((element)=>{
        return element.passing == true;
    })
    renderStudents(sortByPassing,tbody[0]);
})
sorting_buttons[4].addEventListener("click",()=>{

    let sortByClass = studentDB.sort(function(a,b) {
        return a.class -b.class;
    })

    renderStudents(sortByClass,tbody[0]);
})
sorting_buttons[5].addEventListener("click",()=>{

    let malesList = studentDB.filter(Element=>{
    ...
    let femalesList = studentDB.filter(Element=>{
        if(Element.gender == "Female" )
        return Element;
   });
    renderStudents(malesList,tbody[0]);
     renderStudents(femalesList,tbody[1]);
});
